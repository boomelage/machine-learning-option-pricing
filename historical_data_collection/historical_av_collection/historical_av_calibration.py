...
relative_dump_directory = str(f"{symbol}_av_calibrations").replace('.','_')
...
for date,s in spots.items():
    try:
        ...
        test_data.to_csv(os.path.join(relative_dump_directory,f'{symbol} calibrated {calculation_datetime.strftime('%Y-%m-%d')}.csv'))
