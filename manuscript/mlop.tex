...
\usepackage{graphicx}
\usepackage{float}   % [H] pins a table where it is written
\usepackage{parskip}
\usepackage{biblatex}
\usepackage{booktabs}
\usepackage{array}
...
\definecolor{string}{HTML}{800000}     % Dark red for strings%\definecolor{comment}{HTML}{666666} % Medium gray for comments
...
\title{Neural Networks for Exotic Option Pricing}
\author{Radu Briciu}
\date{\today}
...
\begin{titlepage}
	\vfill
	\begin{center}
		\Large Neural Networks for Exotic Option Pricing \\ \vspace{1em}
		\small Radu Briciu \\ \tiny BSc Finance, PgDip Quantitative Finance
	\end{center}
	\vfill
	\begin{abstract}
	\noindent
...
%	\par
%	The complete procedure is made possible with three separate software packages~\cite{optionGenerator2024}~\cite{quantlibPricers2024}~\cite{convsklearn2024}. 
