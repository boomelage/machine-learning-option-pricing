...
Xi['barrier_price'] = barrier_pricer().df_barrier_price(Xi)
...
#
# # The head and tail both sit in blocks where a == t, so n_fixings == 1 and the two
# # averaging types collapse onto the same European payoff. Show the at-the-money
# # strike of the most heavily averaged block in between, or the table demonstrates
# # no averaging at all.
# k = 4
# atm = Xi['strike_price'] == K[len(K)//2]
# averaged = max(
#     (block for _, block in Xi[atm & (Xi['n_fixings'] > 1)].groupby(
#         ['days_to_maturity','fixing_frequency'], sort=False)),
#     key=lambda block: block['n_fixings'].iloc[0])
# Xi = Xi.drop(columns='n_fixings')
# dots = pd.DataFrame(np.tile(' . . . ',(1,len(Xi.columns))),columns=Xi.columns)
# table = pd.concat([Xi.head(k), dots,
#                    averaged.drop(columns='n_fixings'), dots,
#                    Xi.tail(k)])
# print(table)
# print()
# print(table.to_latex())
