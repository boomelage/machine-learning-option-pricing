...
def collect_option_chain_link(date, symbol, key):
    ...
    surf_calls = calls.set_index(['days_to_maturity','strike_price'])
    ...
    for k in K:
        for t in T:
            try:
                call_surf.loc[k,t] = surf_calls.loc[(t,k),'volatility']
            except Exception:
    ...
    surf_puts = puts.set_index(['days_to_maturity','strike_price'])
    ...
    for k in K:
        for t in T:
            try:
                put_surf.loc[k,t] = surf_puts.loc[(t,k),'volatility']
            except Exception:
                put_surf.loc[k,t] = np.nan
    
    option_chain_link['call'] = {
        'surface':call_surf,
        'contracts': calls
        }
    
    option_chain_link['put'] = {
        'surface':put_surf,
        'contracts':puts
        }
